...
TOOLCHAIN_ARCHIVE="${TOOLCHAIN_ARCHIVE:-gcc-11.3-linaro-2022.08-arm-linux-gnueabihf.tar.gz}"
GIT="${GIT:-git}"
...
removeFailingTests_commandArgs() { 
  readarray -t failures < <(sed -e '/^#/d' -e '/^\s*$/d' "disabled_tests.txt")
...
check() {
  local builddir
  builddir="$(readlink -f "${1:-$TOOLCHAIN_BUILDDIR}")"
  ...
  local abedir="$PWD/abe"
  local sysroot="$builddir/builds/destdir/$TOOLCHAIN_HOST_MARCH/$TOOLCHAIN_TARGET_MARCH/libc"
  ...
  # workaround for a gcc11 gcov unit test errors that expect a gcov executable without
  # the arch triple to be located at this (wrong) location within sysroot
  cd "$builddir/builds/destdir/$TOOLCHAIN_HOST_MARCH/bin/" && ln -sf "$TOOLCHAIN_TARGET_MARCH-gcov" gcov

  # workaround for "scan-not-visible" unit test errors that expect cross-objdump in PATH
  local cross_objdump_location="$builddir/builds/destdir/$TOOLCHAIN_HOST_MARCH/bin"
  
  # shellcheck disable=SC2031
  (    export QEMU_LD_PREFIX="$sysroot"; \
       export PATH="$PATH:$cross_objdump_location"; \
       cd "$builddir" \
    && "$abedir/abe.sh" \
          --manifest "$manifest" \
          --set libc="$TOOLCHAIN_LIBC" \
